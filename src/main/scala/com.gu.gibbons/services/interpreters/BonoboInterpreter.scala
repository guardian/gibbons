...
class BonoboInterpreter(config: Settings,
  ...
  def getPotentiallyInactiveDeveloperKeys(createdBefore: Instant) =
    for {
      ...
      keys <- getItems(keysTable,
        not(attributeExists('remindedAt)) and (not(attributeExists('extendedAt)) or ('extendedAt <= millis)) and ('createdAt, millis) and ('tier, "Developer"))
    } yield keys
  ...
  def setRemindedAt(key: Key, when: Long) =
    run {
      keysTable.update('hashkey -> "hashkey" and 'rangekey -> "1634115126000_2c2fd007-c704-43e0-9392-70deb1e9f2d3", set('remindedAt -> when)).map(_ => ())
    }.map { _ =>
