...
class BonoboInterpreter(config: Settings, logger: LoggingService[Task], dynamoClient: AmazonDynamoDBAsync, httpClient: OkHttpClient, urlGenerator: UrlGenerator) extends BonoboService[Task] {
  ...
  def getUsers(jadis: Instant) = for {
    ...
    users <- getItems(usersTable, not(attributeExists('remindedAt)) and ('extendedAt <= millis or (not(attributeExists('extendedAt)) and 'createdAt <= millis)))
  } yield users

  def isDeveloper(users: Vector[User]) = {
    val uss = users.grouped(100).toVector
    for {
      keys <- uss.foldMapM(us => getItems(keysTable, AndCondition('bonoboId -> us.map(u => UserId.unwrap(u.id)).toSet, 'tier -> "Developer")))
  ...
  private def getItems[A, C: ConditionExpression](table: Table[A], filter: C): Task[Vector[A]] = run {
